...
if ( ! function_exists( 'bsf_get_remote_version' ) ) {
	function bsf_get_remote_version( $products, $check_license, $get_from_dt = false) {
		global $ultimate_referer;

		if (!$get_from_dt) $path = dt_get_api_url(false) . '?referer='.$ultimate_referer;
		else $path = dt_get_api_url(false, 'force') . '?referer='.$ultimate_referer;
		...
		if ( is_wp_error( $request ) && wp_remote_retrieve_response_code( $request ) !== 200 ) {
			if (!$get_from_dt)	$path = dt_get_api_url(true) . '?referer='.$ultimate_referer;
			else $path = dt_get_api_url(true, 'force') . '?referer='.$ultimate_referer;
			$request 	= wp_remote_post(
				$path, array(
					'body'    => $data,
					'timeout' => '30',
...
if ( ! function_exists( 'bsf_check_product_update' ) ) {
	function bsf_check_product_update() {
		...
		if(!empty($registered))
		{
			...
			$dt_remote_versions = bsf_get_remote_version($registered, $check_license, true);
